...
mix.js('src/redvolver/js/main.js', 'dist/js');
mix.sass('src/redvolver/sass/main.scss', 'dist/css');
// Global jquery
// mix.autoload({
// 'jquery': ['$', 'jQuery'],
...
mix.sass('src/plugins/plugins.scss', 'dist/plugins/global/plugins.bundle.css').then(() => {
    // remove unused preprocessed fonts folder
    rimraf(path.resolve('dist/fonts'), () => {});
    rimraf(path.resolve('dist/images'), () => {});
}).sourceMaps(!mix.inProduction())
    // .setResourceRoot('./')
    .options({processCssUrls: false}).js(['src/plugins/plugins.js'], 'dist/plugins/global/plugins.bundle.js');

// Metronic css/js
mix.sass('src/metronic/sass/style.scss', 'dist/css/style.bundle.css', {
    sassOptions: {includePaths: ['node_modules']},
})
    // .options({processCssUrls: false})
    .js('src/js/scripts.js', 'dist/js/scripts.bundle.js');

// Custom 3rd party plugins
// (glob.sync('src/plugins/custom/**/*.js') || []).forEach(file => {
//     console.log(file);
//     mix.js(file, `dist/${file.replace('src/', '').replace('.js', '.bundle.js')}`);
// });
// (glob.sync('src/plugins/custom/**/*.scss') || []).forEach(file => {
//     mix.sass(file, `dist/${file.replace('src/', '').replace('.scss', '.bundle.css')}`);
// });

// Metronic css pages (single page use)
// (glob.sync('src/metronic/sass/pages/**/!(_)*.scss') || []).forEach(file => {
//     file = file.replace(/[\\\/]+/g, '/');
//     mix.sass(file, file.replace('src/metronic/sass', 'dist/css').replace(/\.scss$/, '.css'));
// });

// Metronic js pages (single page use)
// (glob.sync('src/metronic/js/pages/**/*.js') || []).forEach(file => {
//     mix.js(file, `dist/${file.replace('src/metronic/', '')}`);
// });

// Metronic media
mix.copyDirectory('src/metronic/media', 'dist/media');

// Metronic theme
// (glob.sync('src/metronic/sass/themes/**/!(_)*.scss') || []).forEach(file => {
//     file = file.replace(/[\\\/]+/g, '/');
//     mix.sass(file, file.replace('src/metronic/sass', 'dist/css').replace(/\.scss$/, '.css'));
// });
...
(glob.sync('src/metronic/plugins/**/*.+(woff|woff2|eot|ttf)') || []).forEach(file => {
    var folder = file.match(/src\/metronic\/plugins\/(.*?)\//)[1];
